...
class TableNav {
	...
	change() {
		...
		const index =
			this.headers.position.length -
			1 -
			this.headers.position_reverse.findIndex(item => item <= window.scrollY)
		if (this.li_list[index] && this.li_list[index].classList.contains('on'))
			return
