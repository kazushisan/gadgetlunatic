...
const scroll = new SmoothScroll('a[href*="#"]', {
	offset: (anchor, toggle) => (
		parseInt(document.defaultView.getComputedStyle(anchor).marginTop.replace('px', '')) + navHeight
	)
})
...
if(toc_element){
	tablenav = new TableNav({
		...
		headers_list: document.querySelectorAll("section.post-content h2, section.post-content h3")
	})
} else if(tfd_element){
	tablenav = new TableNav({
		element: tfd_element,
...
window.addEventListener('scroll', () => {
	...
	ticking = true
	const footerRect = document.querySelector('footer.global-footer').getBoundingClientRect()
	const table_element = toc_element || tfd_element
	
	const bottom = footerRect.y < window.innerHeight ? window.innerHeight - footerRect.y + 'px' : ''
...
window.addEventListener('resize', () => {
	content.style.minHeight = window.innerHeight-global_footer.offsetHeight - 61 + 'px'
})
content.style.minHeight = window.innerHeight-global_footer.offsetHeight - 61 + 'px'
